...
function fit = fit_gaussian_model(results)
% FIT_GAUSSIAN_MODEL Fit Gaussian covariance model to empirical covariance
...
obj = @(th) sum( w .* ...
    (C_emp - cov_gaussian(h, exp(th(1)), exp(th(2)))).^2 );
...
C_fit = cov_gaussian(h, sigma2_hat, a_hat);
...
fit.sse0      = obj(theta0);
fit.sse       = sse;
fit.Lhalf_mod = a_hat * sqrt(log(2));

fit.model_name = 'Gaussian';
fit.model_fun  = @(h) cov_gaussian(h, sigma2_hat, a_hat);
end


% COV_GAUSSIAN Gaussian covariance model
function C = cov_gaussian(h, sigma2, a)

C = sigma2 .* exp(-(h.^2) ./ (a.^2));
end